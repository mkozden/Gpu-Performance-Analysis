# TODO: Get boost clock and architecture from each GPU page (New radeon cards have an even higher "game clock", so account for that as well)
# 		Try multithreading as well
import json
...
class Scraper:
	...
	def get_gpus(self):
		...
		searchbox = self.browser.find_element(By.ID,"quicksearch")
		searchbox.send_keys(" ")
		time.sleep(8)
		table = self.browser.find_element(By.CSS_SELECTOR,"#ajaxresults > table > tbody")
		items = table.find_elements(By.TAG_NAME,"tr")
		items_final = []
		i = 0
		for item in items:
			props = item.find_elements(By.TAG_NAME, "td")
			name = props[0].find_element(By.TAG_NAME, "a").text
	...
	def check_file(self):
		if os.path.exists("gpu_data.json") and os.path.getsize("gpu_data.json") != 0:
			...
			if current_date - os.path.getmtime("gpu_data.json") > 2592000:
				print("The file is older than a month, updating")
				self.get_gpus()
			else:
				self.browser.quit()
